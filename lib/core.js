...
async function createPatternFiles(patternName, formName) {
    switch (patternName) {
        case constants.PATTERNS.BASIC:
            ...
            files.writeFile(formName + '/blocks/toolbar', 'toolbar.form.js', template.getBasicToolbar(formName));
            files.writeFile(formName + '/blocks/details', 'details.form.js', template.getBasicDetails(formName))
            break;
