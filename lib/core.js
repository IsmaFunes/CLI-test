...
async function createPatternFiles(options) {
    ...
    switch (options.patternName) {
        case constants.PATTERNS.BASIC:
            ...
            files.writeFile(options.formName + '/blocks/toolbar', 'toolbar.form.js', template.getBasicToolbar(options));
            files.writeFile(options.formName + '/blocks/details', 'details.form.js', template.getBasicDetails(options))
            break;
